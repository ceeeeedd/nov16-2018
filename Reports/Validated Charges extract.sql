...
--AHMC_DataAnalyticsDB where  validation_id = '2599689'
...
where 
CAST(CONVERT(VARCHAR(10), charge_date,101) as SMALLDATETIME) =  CAST(CONVERT(VARCHAR(10),'10/30/2018',101) as SMALLDATETIME)
order by charge_date desc

select * from dis.df_browse_validated
where
CAST(CONVERT(VARCHAR(10), charge_date,101) as SMALLDATETIME) =  CAST(CONVERT(VARCHAR(10),'10/29/2018',101) as SMALLDATETIME)
order by charge_date desc

select * from dis.df_browse_validated
where  [Change in Status Remarks] is not null
order by charge_date desc

select * from dis.df_browse_validated_updates
--where  validation_id = '2599689'
order by charge_date desc


select * from dbo.costcentre_group
